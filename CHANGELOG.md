...
Fix molecular input caches and expose advanced upstream PySCF/GPU4PySCF controls.

- OrbMol detects charge/multiplicity changes at unchanged coordinates.
  MACE-POLAR detects list/NumPy field replacement and in-place updates.
  NumPy integers are normalized on a computation copy. **Recalculate results
  obtained by sharing an older Orb calculator across electronic-state changes.**
- Add CDIIS/Newton selection, explicit initial guesses and convergence controls.
  Unsupported or ignored settings raise instead of silently doing nothing.
- Add opt-in projected-density reuse and SCF checkpoint restart. Checkpoints
  store completed-iteration orbitals atomically and validate basis/ECP contents,
  electron counts and physical settings. Unconverged restart needs explicit consent.
- Keep failure diagnostics separate from valid ASE results. Record final orbital
  gradients, S², Mulliken spin populations, energy components, effective settings,
  resource dimensions and initial-guess provenance. Optional JSONL streams per iteration.
- Add D3zero and explicit dispersion-functional suffixes, plus PCM/COSMO cavity
  controls with Angstrom input and recorded effective radii.
- Add `get_hessian(atoms)` returning `(3N, 3N)` in eV/Angstrom². SCF retention is
  opt-in; Hessian also works after a force calculation with retention disabled.
  DF auxiliary response uses upstream integer levels 0/1/2, with 2 the full response.
  D3/D4 and SMD retain and document upstream finite-difference correction terms.
- Preserve the default `density_fit: false` on both CPU/GPU and
  `reuse_density: false`. No implicit device/solver fallback or optimizer restart.
- Split the README into installation, API, backend, molecular, dispersion,
  device and validation guides under `docs/`. Correct examples and retain
  prior validation provenance. See the reviewed validation record for numerical
  limits, including upstream CPU/GPU grid-response differences in Hessians.
