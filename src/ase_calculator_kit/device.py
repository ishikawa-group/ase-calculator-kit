...
def resolve_device(device: str = "auto", *, allow_mps: bool = False) -> str:
    ...
    allow_mps:
        When ``True`` (CHGNet, TensorNet, SevenNet, or MatterSim on Apple Silicon),
    ...
    Notes
    -----
    ``torch`` is imported lazily so that importing :mod:`ase_calculator_kit`
    does not require a torch installation until a calculator is actually built.
    """
